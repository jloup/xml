...
func testEntryValidator(actual helper.Visitor, expected helper.Visitor) error {
	e1 := actual.(*Entry)
	e2 := expected.(*Entry)
...
func testEntryConstructor() helper.Visitor {
	return NewEntry()
}

func _TestEntryToTestVisitor(t testEntry) helper.TestVisitor {
...
`,
			helper.NewError(MissingSummary, ""),
			NewTestEntry(
				[]*Person{
					NewTestPerson("Jean", "", ""),
				},
